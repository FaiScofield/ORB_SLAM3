...
public:
    ...
    std::vector<size_t> GetLinesInArea(const float &x1, const float &y1, const float &x2, const float &y2, const float &r, const int minLevel=-1, const int maxLevel=-1, const float TH = 0.998) const;
    ...
    cv::Mat GetPoseInverse() const { return mTcw.inv(); };
...
public:
    ...
    std::vector<float> mvScaleFactorsLine;
    std::vector<float> mvInvScaleFactorsLine;
    std::vector<float> mvLevelSigma2Line;
    std::vector<float> mvInvLevelSigma2Line;
    
    int NL;          // 特征线个数
    cv::Mat mLdesc;  //线特征描述子
