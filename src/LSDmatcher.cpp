...
LSDmatcher::LSDmatcher(float nnratio, bool checkOri) :
...
int LSDmatcher::SearchByProjection(Frame& CurrentFrame, const Frame& LastFrame)
{
    ...
    for (int i = 0, ie = (int)tempMatches1.size(); i < ie; i++) {
    ...
    for (int i = 0; i < LastFrame.NL; i++) {
        ...
        if (pML) {
            if (!LastFrame.mvbLineOutlier[i]) {
                ...
                vIndices2 = CurrentFrame.GetFeaturesInAreaForLine(pML->mTrackProjX1, pML->mTrackProjY1,
                                                                  pML->mTrackProjX2, pML->mTrackProjY2, radius,
                                                                  nLastOctave - 1, nLastOctave + 1, 0.96);
                ...
                for (vector<size_t>::const_iterator vit = vIndices2.begin(), vend = vIndices2.end();
                    ...
                    float max_ = std::max(LastFrame.mvKeylinesUn[i].lineLength,
                                          CurrentFrame.mvKeylinesUn[i2].lineLength);
                    float min_ = std::min(LastFrame.mvKeylinesUn[i].lineLength,
                                          CurrentFrame.mvKeylinesUn[i2].lineLength);
                ...
                if (bestDist <= TH_HIGH) {
                    ...
                    Point_1.x = (line_1.startPointX + line_1.endPointX) / 2.0;
                    Point_1.y = (line_1.startPointY + line_1.endPointY) / 2.0;
                    Point_2.x = (line_2.startPointX + line_2.endPointX) / 2.0 +
                                CurrentFrame.ImageGray.cols;
                    Point_2.y = (line_2.startPointY + line_2.endPointY) / 2.0;
                    cv::line(pic_Temp, Point_1, Point_2, Scalar(0, 0, 255), 1, CV_AA);
                    /////////////////////////////////////////////////////////////////////////////////////////////////////
                }
...
int LSDmatcher::SearchByProjection(Frame& F, const std::vector<MapLine*>& vpMapLines, const float th)
{
    ...
    for (size_t iML = 0; iML < vpMapLines.size(); iML++) {
        ...
        vector<size_t> vIndices = F.GetFeaturesInAreaForLine(pML->mTrackProjX1, pML->mTrackProjY1,
                                                             pML->mTrackProjX2, pML->mTrackProjY2,
                                                             r, nPredictLevel - 1, nPredictLevel);
...
int LSDmatcher::SerachForInitialize(Frame& InitialFrame, Frame& CurrentFrame, vector<int>& LineMatches)
{
...
#if ENBALE_DEBUG_DATA_LSDMATCHER
    Mat tmp1, tmp2, tmpShow;
    cvtColor(InitialFrame.ImageGrayUn, tmp1, COLOR_GRAY2BGR);
    cvtColor(CurrentFrame.ImageGrayUn, tmp2, COLOR_GRAY2BGR);
    for (auto iter = InitialFrame.mvKeylinesUn.begin(); iter != InitialFrame.mvKeylinesUn.end(); ++iter) {
        line(tmp1, iter->getStartPoint(), iter->getEndPoint(), Scalar(255,0,0), 1, LINE_AA);
    }
    for (auto iter = CurrentFrame.mvKeylinesUn.begin(); iter != CurrentFrame.mvKeylinesUn.end(); ++iter) {
        line(tmp2, iter->getStartPoint(), iter->getEndPoint(), Scalar(255,0,0), 1, LINE_AA);
    }
    hconcat(tmp1, tmp2, tmpShow);

    Point2f pt1, pt2;
    const int xLimit = InitialFrame.ImageGrayUn.cols;
    const int yLimit = InitialFrame.ImageGrayUn.rows;
#endif
...
#else
    BFMatcher* bfm = new BFMatcher(NORM_HAMMING, true);
    vector<DMatch> lmatches;
    bfm->match(ldesc1, ldesc2, lmatches);
    for (size_t i = 0; i < lmatches.size(); i++) {
        const int qdx = lmatches[i].queryIdx;
        const int tdx = lmatches[i].trainIdx;
        LineMatches[qdx] = tdx;
        nmatches++;
    }
#endif

#if ENBALE_REMOVE_OUTLIER_WITH_RANSAC
    vector<Point2f> vpts1, vpts2;
    vector<int> vidx;
    vpts1.reserve(LineMatches.size());
    vpts2.reserve(LineMatches.size());
    vidx.reserve(LineMatches.size());
    for (int i = 0, ie = LineMatches.size(); i < ie; ++i) {
        if (LineMatches[i] < 0)
            continue;

        vpts1.push_back(InitialFrame.mvKeylinesUn[i].pt);
        vpts2.push_back(CurrentFrame.mvKeylinesUn[LineMatches[i]].pt);
        vidx.push_back(i);
    }
    if (vpts1.size() > 10) {
        vector<uchar> vInlier;
        estimateAffinePartial2D(vpts1, vpts2, vInlier, RANSAC);
        for (int i = 0, ie = vInlier.size(); i < ie; ++i) {
            if (!vInlier[i]) {
                LineMatches[vidx[i]] = -1;
                nmatches--;
            }
        }
    } else {
        cout << "WARN: Not enough matches for RANSAC! " << vpts1.size() << endl;
    }
    ...
    for (int i = 0, ie = LineMatches.size(); i < ie; ++i) {
        ...
        pt1 = InitialFrame.mvKeylinesUn[i].pt;
        pt2 = CurrentFrame.mvKeylinesUn[LineMatches[i]].pt;
        if (pt1.x < 0 || pt1.y < 0 || pt1.x >= xLimit || pt1.y >= yLimit ||
            pt2.x < 0 || pt2.y < 0 || pt2.x >= xLimit || pt2.y >= yLimit)
            continue;
        pt2.x += xLimit;
        line(tmpShow, pt1, pt2, Scalar(0,0,255), 2, LINE_AA);
    }
    imshow("KL matches after RANSAC", tmpShow);
    char filename[256];
    snprintf(filename, 256, "/home/vance/output/KL_matches/kl_matches_after_remove_outlier_%03d_to_%03d.jpg", InitialFrame.mnId, CurrentFrame.mnId);
    imwrite(filename, tmpShow);
    waitKey(10);
#endif // ENBALE_DEBUG_DATA_LSDMATCHER

#endif // ENBALE_REMOVE_OUTLIER_WITH_RANSAC
...
int LSDmatcher::SearchDouble(KeyFrame* KF, Frame& CurrentFrame)
{
    ...
    for (int i = 0, ie = (int)tempMatches2.size(); i < ie; i++) {
...
int LSDmatcher::SearchDouble(Frame& InitialFrame, Frame& CurrentFrame, vector<int>& LineMatches)
{
    ...
    const int xLimit = InitialFrame.ImageGrayUn.cols;
    const int yLimit = InitialFrame.ImageGrayUn.rows;
    for (int i = 0, ie = tempMatches1.size(); i < ie; ++i) {
        if (tempMatches1[i] < 0)
            continue;

        pt1 = InitialFrame.mvKeylinesUn[i].pt;
        pt2 = CurrentFrame.mvKeylinesUn[tempMatches1[i]].pt;
        if (pt1.x < 0 || pt1.y < 0 || pt1.x >= xLimit || pt1.y >= yLimit ||
            pt2.x < 0 || pt2.y < 0 || pt2.x >= xLimit || pt2.y >= yLimit)
            continue;
        pt2.x += xLimit;
        line(tmpShow, pt1, pt2, Scalar(0,255,0), 1, LINE_AA);
    }
#endif
    ...
    vector<Point2f> vpts1, vpts2;
    vector<int> vidx;
    vpts1.reserve(tempMatches1.size());
    vpts2.reserve(tempMatches1.size());
    vidx.reserve(tempMatches1.size());
    for (int i = 0, ie = tempMatches1.size(); i < ie; ++i) {
        if (tempMatches1[i] < 0)
            continue;

        vpts1.push_back(InitialFrame.mvKeylinesUn[i].pt);
        vpts2.push_back(CurrentFrame.mvKeylinesUn[tempMatches1[i]].pt);
        vidx.push_back(i);
    }
    if (vpts1.size() > 10) {
        vector<uchar> vInlier;
...
void LSDmatcher::FrameBFMatch(cv::Mat ldesc1, cv::Mat ldesc2, vector<int>& LineMatches, float TH)
{
    ...
    for (size_t i = 0; i < lmatches.size(); i++) {
        ...
        double dist_12 = lmatches[i][1].distance - lmatches[i][0].distance;
        if (dist_12 > nn12_dist_th && lmatches[i][0].distance < TH &&
            lmatches[i][0].distance < mfNNratio * lmatches[i][1].distance)
            LineMatches[qdx] = tdx;
    }
}

void LSDmatcher::FrameBFMatchNew(cv::Mat ldesc1, cv::Mat ldesc2, vector<int>& LineMatches,
                                 vector<KeyLine> kls1, vector<KeyLine> kls2,
                                 vector<Eigen::Vector3d> kls2func, cv::Mat F, float TH)
{
    LineMatches = vector<int>(ldesc1.rows, -1);
    ...
    for (size_t i = 0; i < lmatches.size(); i++) {
        for (size_t j = 0; j < lmatches[i].size() - 1; j++) {
...
void LSDmatcher::lineDescriptorMAD(vector<vector<DMatch>> line_matches, double& nn_mad, double& nn12_mad) const
{
    ...
    nn12_dist_median = matches_12[int(matches_12.size() / 2)][1].distance -
                       matches_12[int(matches_12.size() / 2)][0].distance;
...
int LSDmatcher::SearchForTriangulation(KeyFrame* pKF1, KeyFrame* pKF2, vector<pair<size_t, size_t>>& vMatchedPairs)
{
...
int LSDmatcher::SearchForTriangulationNew(KeyFrame* pKF1, KeyFrame* pKF2, vector<int>& vMatchedPairs, bool isDouble)
{
    ...
    thread thread12(&LSDmatcher::FrameBFMatchNew, this, ldesc1, ldesc2, std ::ref(tempMatches1),
                    kls1, kls2, kls2func, F21, TH_LOW);
    thread thread21(&LSDmatcher::FrameBFMatchNew, this, ldesc2, ldesc1, std ::ref(tempMatches2),
                    kls2, kls1, kls1func, F12, TH_LOW);
    thread12.join();
    thread21.join();
