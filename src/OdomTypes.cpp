...
void Calib::Set(const cv::Mat &Tbc_, float noise)
{
    Tbc = Tbc_.clone();
    ...
    Cov = cv::Mat::eye(3,3,CV_32F);
    const float ng2 = noise * noise;
...
Point Point::operator+(const Point& that) const
{
    ...
    float _x = data.x + that.data.x * c - that.data.y * s;
    float _y = data.y + that.data.x * s + that.data.y * c;
    float _theta = NormalizeAngle(data.z + that.data.z);
    return Point(_x, _y, _theta, t);
}

Point Point::operator-(const Point& that) const
{
    float dx = data.x - that.data.x;
    float dy = data.y - that.data.y;
    float dth = NormalizeAngle(data.z - that.data.z);

    float c = std::cos(that.data.z);
    float s = std::sin(that.data.z);
    return Point(c * dx + s * dy, -s * dx + c * dy, dth);
}
...
Point invOdom(const Point& p)
{
    ...
    float s = std::sin(p.data.z);
    return Point(-c * p.data.x - s * p.data.y, s * p.data.x - c * p.data.y, -p.data.z, p.t);
}
